...
async function createClient() {
    ...
    if (!configHandler.isConfigured()) {
        ...
        throw new Error('Database connection not configured. Please use the "config" tool to set up your PostgreSQL connection:\n' +
    ...
    if (!config.password || config.password === '') {
        ...
        throw new Error('Database password not set. Please configure your PostgreSQL connection first using the "config" tool.');
    }
 ...
 */
function isValidQueryArgs(args) {
 ...
 */
function isValidTableInfoArgs(args) {
    return (typeof args === 'object' &&
        args !== null &&
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
      ...
      `, [schema]);
            ...
            if (error instanceof McpError)
                throw error;
            const errorMessage = error instanceof Error ? error.message : String(error);
            throw new McpError(ErrorCode.InternalError, `Database error while reading resource "${uri}": ${errorMessage}`);
        }
        finally {
            await client.end();
        }
