...
export class McpConfigHandler {
     ...
     */
    constructor(hideConfigTools = false) {
    ...
    async handleConfig(args) {
        ...
        try {
            const { clients, host, port, database, user, password, ssl, connectionTimeout, queryTimeout, description, global = false } = args;
...
You can now use other database tools to interact with your PostgreSQL database.`
    ...
    async handleConfigGet() {
        ...
        try {
            ...
            const postgresKeys = keys.filter((key) => key.startsWith('postgres.'));
...
${JSON.stringify(configInfo, null, 2)}`
     ...
     */
    async handleGetSupportedClients() {
        try {
            const supportedClients = ['VS Code', 'Claude Code', 'Claude Desktop', 'Cursor'];
            const clientsResult = getConfigValue('clients.selected');
